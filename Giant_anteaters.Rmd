...
```{r}
stations <- read.csv('dep_piedemonte.csv')
...
stations1 <- read.csv('dep_sabana.csv')
...
rm(NDVI21,NDVI20,NDVI_CT1,NDVI_CT2,lulc_CT,ghmct,ghm,lulc,roads,roads_ed,slope,fcov,fcover)
```
...
pdm <- read.csv('hormigueros_pdm.csv')  ##data base with species, dates and stations
sab <- read.csv('hormigueros_sab.csv')
...
rm(check,check2,buffers,cams,CT_shp,CT_elev,CT_buf,CT_slope,df,dfpdm,dfs,dfunique,dh,dhpdm,pdm,ps1,ps2,RecordTaSA,rest,sab,sarea,stations,stations1,x,elev)
...
```{r}
ylist <- list(GiantAnteater = as.matrix(GA[,17:27]),
              SouthernTamandua = as.matrix(TA[,17:27]))
...
stateformulas = list(smod1 = c("~scale(elev)","~scale(elev)","~1"),
                     smod2 = c("~scale(ghm)","~scale(elev)","~1"),
                     smod3 = c("~scale(roads)","~scale(elev)","~scale(ghm)"))

detformulas = list(dmod1 = c("~1","~1"),
                   dmod2 = c("~1","~1"))

mod1 <- occuMulti(detformulas=detformulas$dmod1, stateformulas=stateformulas$smod1, data=umf)
mod1 # estimates on logit scale

mod2 <- occuMulti(detformulas=detformulas$dmod2, stateformulas=stateformulas$smod2, data=umf)
mod2 # estimates on logit scale

mod3 <- occuMulti(detformulas=detformulas$dmod2, stateformulas=stateformulas$smod3, data=umf)
mod3 # estimates on logit scale

# create a list to compare the models
models = list(mod1,mod2,mod3)
# AIC values for models
model.selection <- aictab(models)
...
## plotting results
```{r}
occ_prob <- predict(mod1, type="state")

lapply(occ_prob, head)

head(apply(occ_prob$Predicted, 1, sum))

GA_marginal <- predict(mod1, type="state", species="GiantAnteater")
GA_marginal


TA_marginal <- predict(mod1, type='state', species="SouthernTamandua") # get coyote
marg_plot_dat <- rbind(GA_marginal[1,], TA_marginal[1,])
marg_plot_dat$Species <- c("GiantAnteater", "SouthernTamandua")
marg_plot_dat


plot(1:2, marg_plot_dat$Predicted, ylim=c(0.1,1.0), 
...
GA_TA <- predict(mod1, type="state", species="GiantAnteater", cond="SouthernTamandua")
head(GA_TA)

GA_noTA <- predict(mod1, type="state", species="GiantAnteater", cond="SouthernTamandua")
head(GA_noTA)


plot_data <- rbind(GA_TA[1,], GA_noTA[1,])
plot_data$SouthernTamandua_status <- c("Present","Absent")
head(plot_data)

plot(1:2, plot_data$Predicted, ylim=c(0, 1.0), 
     xlim=c(0.5,2.5), pch=19, cex=1.5, xaxt='n', 
     xlab="Tamandua status", ylab="Giant anteater conditional occupancy and 95% CI")
axis(1, at=1:2, labels=plot_data$SouthernTamandua_status)
...
# try the opposite
GA_TA <- predict(mod1, type="state", species="SouthernTamandua", cond="GiantAnteater")
head(GA_TA)

GA_noTA <- predict(mod1, type="state", species="SouthernTamandua", cond="GiantAnteater")
head(GA_noTA)
...
plot(1:2, plot_data$Predicted, ylim=c(0, 1.0), 
     xlim=c(0.5,2.5), pch=19, cex=1.5, xaxt='n', 
     xlab="Giant Anteater status", ylab="Tamandua conditional occupancy and 95% CI")
axis(1, at=1:2, labels=plot_data$SouthernTamandua_status)
